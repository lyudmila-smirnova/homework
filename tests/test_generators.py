...
def mixed_transactions():
    ...
    return [
        # Перевод организации
        {"id": 1, "from": "Организация Рога и копыта", "to": "счет 1234567890"},
        # Перевод со счета на счет
        {"id": 2, "from": "счет 1111111111", "to": "счет 2222222222"},
        # Обычный перевод (без специфических признаков)
        {"id": 3, "from": "неизвестный отправитель", "to": "неизвестный получатель"},
    ]


@pytest.mark.parametrize(
    "transaction_index, expected_description",
    [
